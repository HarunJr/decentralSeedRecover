...
function getInputTx() {
	...
	SELECTED_WALLET_ADDR=$(cat $BASE/.priv/wallets/preview/$SELECTED_WALLET_NAME/$SELECTED_WALLET_NAME.payment.addr)
	cat $BALANCE_FILE
...
walletAddress() {
	WALLET_ADDRESS=$(cat $BASE/.priv/wallets/preview/$1/$1.payment.addr)
}
