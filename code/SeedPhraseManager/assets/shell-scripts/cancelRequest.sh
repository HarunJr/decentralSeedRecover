...
MINT_SCRIPT_ADDRESS=$($CARDANO_CLI address build --payment-script-file $SCRIPT_FILE --testnet-magic $TESTNET_MAGIC)
...
MINT_REDEEMER_FILE=$'cancel-request-redeemer.json'
...
# FEE_ADDR=${TX_IN_ARRAY[0]}
# FEE_ADDR=($FEE_ADDR)
# echo "---FEE_ADDR---" ${FEE_ADDR}
...
case $input in
    ...
    [nN][oO]|[nN])
        ...
        --mint-redeemer-file $WORK/redeemer/${MINT_REDEEMER_FILE} \
        ${REQUIRED_SIGNER_ARRAY} \
        --protocol-params-file $WORK/transactions/pparams.json \
        --out-file $WORK/transactions/tx.draft)
        ;;
